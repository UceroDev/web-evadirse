...
export const POST: APIRoute = async ({ request, redirect }) => {
  ...
  if (
    ...
    !apellido ||
    !correoValido ||
    !OPCIONES_VALIDAS.includes(reserva)
  ) {
  ...
  try {
    ...
    return new Response("No se pudo verificar que eres humano", {
      status: 400,
    });
  }
  ...
  try {
    ...
    msg.setHeader(
      "Reply-To",
      new Mailbox({ name: `${nombre} ${apellido}`, addr: correo }),
    );
    ...
    return new Response("Error al enviar el correo, inténtalo de nuevo", {
      status: 502,
    });
  }
